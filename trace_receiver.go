...
package zipkinreceiverhealthcheckable // import "github.com/open-telemetry/opentelemetry-collector-contrib/receiver/zipkinreceiverhealthcheckable"
...
import (
	...
	"github.com/rvs-fluid-it/zipkinreceiverhealthcheckable/internal/healthcheck"
)
...
// getEndpoints specifies the list of endpoints registered for the zipkin receiver.
func (zr *zipkinReceiver) getEndpoints() []Endpoint {
	endpoints := []Endpoint{
		{
			Pattern: "/health",
			Handler: zr.healthCheck.Handler().ServeHTTP,
		},
		{
			Pattern: "/",
			Handler: zr.ServeHTTP,
		},
	}
...
func (zr *zipkinReceiver) Start(ctx context.Context, host component.Host) error {
	...
	zr.healthCheck = healthcheck.New()
	zr.healthCheck.Set(healthcheck.Ready)
	endpoints := zr.getEndpoints()
	...
	zr.server, err = zr.config.ServerConfig.ToServer(ctx, host, zr.settings.TelemetrySettings, ddmux)
	if err != nil {
		return err
	}
